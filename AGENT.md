...
코드를 수정하기전 어떻게 수정할 것인지 요약해서 확인을 받음 
허락을 받으면 코드를 수정한다
...
10. 위치, 속도, 크기 등 간단한 수정이 필요한 부분은 따로 주석으로 따로 표시하기

- 기존 구조를 최대한 유지한다.
- GameManager가 씬 전환을 담당한다.
- 각 Scene은 자기 입력, 업데이트, 렌더링만 담당한다.
- Render 안에서 이미지를 Load하지 않는다.
- 이미지는 Initialize에서 Load하고 Shutdown에서 Destroy한다.
- 기존 파일을 크게 갈아엎지 말고 필요한 부분만 수정한다.
- 팀원이 만든 미니게임이나 Scene 파일은 요청 없이는 수정하지 않는다.

## 미니게임4 방향
- 파일명: SchoolRun.h / SchoolRun.cpp
- 클래스명: SchoolRunScene
- 게임 이름: 윤서의 등교길
- 장르: 쿠키런식 횡스크롤 러닝 액션
- 조작: Space 점프, S 또는 아래 방향키 슬라이드
- 목표: 장애물을 피하며 최대 거리 달성
- 결과: 거리에 따라 점수 반환
